...
$post_types      = array( 'attachment', 'wpcf7_contact_form', 'post', 'page', 'property' );
	...
	foreach ( $post_datas as $post_data ) {
		...
		foreach ( $meta as $meta_key => $meta_val ) {
			if (
				...
				(
					...
					strpos( $meta_key, 'findeo_page_layout' ) === false &&
					strpos( $meta_key, 'findeo_titlebar' ) === false &&
					strpos( $meta_key, 'findeo_subtitle' ) === false &&
					strpos( $meta_key, 'vc_' ) === false &&
					strpos( $meta_key, 'wpb_' ) === false &&
					strpos( $meta_key, 'findeo_' ) === false &&
					...
					! in_array( $meta_key, array(
						...
						'findeo_titlebar',
						'findeo_subtitle',
						'findeo_parallax_image',
						'findeo_parallax_color',
						'findeo_parallax_opacity',
						'findeo_sidebar_select',
						'findeo_revslider',
						'findeo_project_date',
						'findeo_project_location',
						'findeo_gallery_type',
						'findeo_team_position',
						'_gallery',
						'_wp_attached_file',
						'_wp_page_template',
						'_thumbnail_id',
		...
		if ( $post_type == 'attachment' ) {
			...
			if ( is_file( $file ) ) {
				...
				if ( is_dir( get_home_path() . '/wp-content/uploads/findeo/images/' ) ) {
					
					copy( $file, get_home_path() . '/wp-content/uploads/findeo/images/' . basename( $file ) );
				}
			}
...
foreach ( $all_options as $name => $value ) {
	...
	if ( 'theme_mods_theme-findeo' === $name ) {
		$my_options[ $name ] = maybe_unserialize($value);
...
if ( is_dir( get_home_path() . '/wp-content/uploads/findeo/' ) ) {
	
	file_put_contents( get_home_path() . '/wp-content/uploads/findeo/default.json' , json_encode( $default_content ) );
	file_put_contents( get_home_path() . '/wp-content/uploads/findeo/widget_positions.json' , json_encode( $widget_positions ) );
	file_put_contents( get_home_path() . '/wp-content/uploads/findeo/widget_options.json' , json_encode( $widget_options ) );
	file_put_contents( get_home_path() . '/wp-content/uploads/findeo/menu.json' , json_encode( $menu_ids ) );
	file_put_contents( get_home_path() . '/wp-content/uploads/findeo/options.json' , json_encode( $my_options ) );
}
