...
/**
 * Realteo_Paid_Listings_Subscriptions
 */
class Realteo_Paid_Listings_Subscriptions {
	...
	public function __construct() {
		if ( class_exists( 'WC_Subscriptions_Synchroniser' ) && method_exists( 'WC_Subscriptions_Synchroniser', 'save_subscription_meta' ) ) {
			add_action( 'woocommerce_process_product_meta_property_package_subscription', 'WC_Subscriptions_Synchroniser::save_subscription_meta', 10 );
		}
	...
	public function updated_post_meta( $meta_id, $object_id, $meta_key, $meta_value ) {
		if ( false !== $this->get_listing_subscription_order_id( $object_id )
		     && '' !== $meta_value
		     && '_property_expires' === $meta_key
		) {
			update_post_meta( $object_id, '_property_expires', '' ); // Never expire automatically
		}
	}
	 ...
	 */
	private function get_listing_subscription_order_id( $property_id ) {
		if ( 'property' === get_post_type( $property_id ) ) {
			$user_package_id = get_post_meta( $property_id, '_user_package_id', true );
			$user_package    = realteo_user_packages( $user_package_id );
			$package_id      = get_post_meta( $property_id, '_package_id', true );
			$package         = wc_get_product( $package_id );
			if ( $user_package
			     && $user_package->has_package()
				 && ( $package instanceof WC_Product_Property_Package_Subscription)
		...
		if ( 'property' === $post->post_type ) {
			...
			if ( $package_product_id ) {
				...
				if ( 'listing' === $subscription_type ) {
					$new_count = $wpdb->get_var( $wpdb->prepare( "SELECT package_count FROM {$wpdb->prefix}realteo_user_packages WHERE id = %d;", $package_id ) );
					$new_count --;
		...
		if ( $id > 0 ) {
			...
			if ( 'property' === $post_type) {
				...
				if ( $package_product_id ) {
					...
					if ( 'listing' === $subscription_type ) {
						$package  = $wpdb->get_row( $wpdb->prepare( "SELECT package_count, package_limit FROM {$wpdb->prefix}realteo_user_packages WHERE id = %d;", $package_id ) );
	...
	public function subscription_ended( $subscription, $paused = false ) {
		...
		foreach ( $subscription->get_items() as $item ) {
			...
			$user_package      = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}realteo_user_packages WHERE order_id IN ( %d, %d ) AND product_id = %d;", $subscription->get_id(), $legacy_id, $item['product_id'] ) );
			...
			if ( $user_package ) {
				...
				if ( 'listing' === $subscription_type ) {
					$listing_ids = realteo_get_listings_for_package( $user_package->id );
		...
		delete_post_meta(  $subscription->get_id(), 'realteo_subscription_packages_processed' );
	}
	...
	public function subscription_activated( $subscription ) {
		...
		if ( get_post_meta( $subscription->get_id(), 'realteo_subscription_packages_processed', true ) ) {
			return;
		}
		...
		$parent_id         = ! empty( $parent ) ?  $parent->get_id() : null;
		$legacy_id         = isset( $parent_id ) ? $parent_id : $subscription->get_id();
		$wpdb->delete( "{$wpdb->prefix}realteo_user_packages", array(
			'order_id' => $legacy_id,
		) );
		$wpdb->delete( "{$wpdb->prefix}realteo_user_packages", array(
		...
		foreach ( $subscription->get_items() as $item ) {
			...
			if ( $product->is_type( array( 'property_package_subscription' ) ) && $subscription->get_user_id() && ! isset( $item['switched_subscription_item_id'] ) ) {
				...
				for ( $i = 0; $i < $item['qty']; $i ++ ) {
					$user_package_id = realteo_give_user_package( $subscription->get_user_id(), $product->get_id(), $subscription->get_id() );
				}
				...
				foreach ( $listing_ids as $listing_id ) {
					if ( in_array( get_post_status( $listing_id ), array( 'pending_payment', 'expired' ) ) ) {
						realteo_approve_listing_with_package( $listing_id, $subscription->get_user_id(), $user_package_id );
		...
		update_post_meta( $subscription->get_id(), 'realteo_subscription_packages_processed', true );
	}
		...
		foreach ( $subscription->get_items() as $item ) {
			...
			if ( 'package' === $subscription_type ) {
				if ( ! $wpdb->update(
					...
					realteo_give_user_package( $subscription->get_user_id(), $item['product_id'], $subscription->get_id() );
				}
			} else {
				// Otherwise the listings stay active, but we can ensure they are synced in terms of featured status etc
				if ( $user_package_ids = $wpdb->get_col( $wpdb->prepare( "SELECT id FROM {$wpdb->prefix}realteo_user_packages WHERE order_id IN ( %d, %d ) AND product_id = %d;", $subscription->get_id(), $legacy_id, $item['product_id'] ) ) ) {
					foreach ( $user_package_ids as $user_package_id ) {
						$package = realteo_get_user_package( $user_package_id );

						if ( $listing_ids = realteo_get_listings_for_package( $user_package_id ) ) {
							foreach ( $listing_ids as $listing_id ) {
								// Featured or not
	...
	public function switch_package( $user_id, $new_subscription, $old_subscription ) {
		...
		$user_package      = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}realteo_user_packages WHERE order_id IN ( %d, %d ) AND product_id = %d;", $old_subscription->id, $legacy_id, $old_subscription->product_id ) );
		...
		if ( $user_package ) {
			...
			$switching_to_package_id = realteo_give_user_package( $user_id, $new_subscription->product_id, $new_subscription->id );
			...
			if ( $new_subscription->product instanceof WC_Product_Property_Package_Subscription ) {
				$does_new_subscription_feature_listings = $new_subscription->product->is_property_featured();
			}
...
Realteo_Paid_Listings_Subscriptions::get_instance();
