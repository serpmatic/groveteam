...
class Realteo_Paid_Properties {
	...
	public function __construct() {
		...
		add_action( 'woocommerce_process_product_meta_property_package_subscription', array( $this, 'save_package_data' ) );
	...
	function realteo_add_custom_settings() {
	    ...
	    echo '<div class="options_group show_if_property_package show_if_property_package_subscription">';

	    woocommerce_wp_select( array(
		'id' => '_property_package_subscription_type',
		'wrapper_class' => 'hide_if_property_package show_if_property_package_subscription',
		'label' => __( 'Subscription Type', 'realteo' ),
		'description' => __( 'Choose how subscriptions affect this package', 'realteo' ),
		'value' => get_post_meta( $post->ID, '_package_subscription_type', true ),
		'desc_tip' => true,
		'options' => array(
			'package' => __( 'Link the subscription to the package (renew listing limit every subscription term)', 'realteo' ),
			'listing' => __( 'Link the subscription to posted listings (renew posted listings every subscription term)', 'realteo' ),
		),
	) );
		...
		jQuery(function(){
			...
			jQuery('._tax_status_field').closest('div').addClass( 'show_if_property_package show_if_property_subscription' );
			jQuery('.show_if_subscription, .options_group.pricing').addClass( 'show_if_property_package_subscription' );
			jQuery('.options_group.pricing ._regular_price_field').addClass( 'hide_if_property_package_subscription' );
			
			jQuery('#product-type').change();
			jQuery('#_property_package_subscription_type').change(function(){
				if ( jQuery(this).val() === 'listing' ) {
					jQuery('#_property_duration').closest('.form-field').hide().val('');
				} else {
					jQuery('#_property_duration_duration').closest('.form-field').show();
				}		
			}).change();
		});
	</script>
